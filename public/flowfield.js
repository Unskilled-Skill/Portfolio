...
(function () {
  function initFlowField(canvas) {
    ...
    function loop() {
      ...
      if (shocks.length) shocks = shocks.filter(function (s) { return now - s.born < 745; });
      ...
      for (var i = 0; i < particles.length; i++) {
        ...
        for (var si = 0; si < shocks.length; si++) {
          ...
          var ringDist = Math.abs(sd - sAge * 360);
          if (ringDist < 66) {
            var sf = (1 - ringDist / 66) * (1 - sAge / 0.74);
            if (sf > 0) {
              var dirn = sk.golden ? -1 : 1;
              // radial outward shove
              vx += (sdx / sd) * sf * 3.6; vy += (sdy / sd) * sf * 3.6;
              // tangential swirl → spins the field into a vortex around the blast
              vx += (-sdy / sd) * sf * 6 * dirn; vy += (sdx / sd) * sf * 6 * dirn;
      ...
      for (var pi = pops.length - 1; pi >= 0; pi--) {
        ...
        var radius = 9 + prog * 88;  // expanding pulse
        ctx.save();
